import React, { useState, useEffect, useRef, memo, Dispatch, SetStateAction, HTMLAttributes } from "react";
...
interface LocationMarkerProps extends HTMLAttributes<HTMLElement> {
...
interface MapBBoxHandlerProps {
  setZoom: Dispatch<SetStateAction<number>>;
  setBbox: Dispatch<SetStateAction<[number, number, number, number]>>;
  fetchStations: (bbox: [number, number, number, number]) => Promise<void>;
  zoom: number;
  bbox: [number, number, number, number];
  minZoomLevel: number;
  prevBboxRef: React.RefObject<[number, number, number, number]>;
}
...
interface ChooseLocationProps {
  ...
  setLatInput: Dispatch<SetStateAction<string>>;
  setLngInput: Dispatch<SetStateAction<string>>;
  setShowLocationDiv: Dispatch<SetStateAction<boolean>>;
  latInput: string;
  lngInput: string;
}
...
const LocationMarker = memo(
  ({ coordinates, setCoordinates, setLatInput, setLngInput }: LocationMarkerProps) => {
...
const MapUpdater = ({ coordinates }: MapUpdaterProps) => {
...
const MapBBoxHandler = ({
  ...
  useEffect(() => {
    ...
    void updateStations();
  }, [zoom, bbox, fetchStations, minZoomLevel, prevBboxRef]);
...
function ChooseLocation({
  ...
  useEffect(() => {
    ...
    // Use type assertion to safely access private property
  ...
  const prevBboxRef = useRef<[number, number, number, number]>(bbox);

  // showOptionsDiv: Boolean to show the options div. Used to toggle the options div.
  const [showOptionsDiv, setShowOptionsDiv] = useState<boolean>(false);

  // cligenVersion: The version of the Cligen model. Used in the Options dropdown.
  const [cligenVersion, setCligenVersion] = useState<string>("5.3.2");

  // databaseVersion: The version of the database. Used in the Options dropdown.
  const [databaseVersion, setDatabaseVersion] = useState<string>("legacy");

  // Fetch stations based on the bounding box.
  const fetchStations = async (bbox: [number, number, number, number]): Promise<void> => {
    try {
      // API call
      const response = await api.post<StationFeature>("/api/rockclim/GET/stations_geojson", {
